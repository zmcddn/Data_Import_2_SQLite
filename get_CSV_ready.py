...
class PrepareCSV:
	...
	def spilt_columns(self):
		...
		self.data[special_attentation_columns[0]] = self.data[special_attentation_columns[0]].str.replace('(?:\s|^|\d)dr(?:\s)', '')
		self.data[special_attentation_columns[0]] = self.data[special_attentation_columns[0]].str.replace('Sport Utility', 'Sport Utility Vehicle')
		self.data[special_attentation_columns[0]] = self.data[special_attentation_columns[0]].str.strip()
		# print(self.data[special_attentation_columns[0]])

		# Extract engine displacement
		self.data[special_attentation_columns[1]] = self.data[special_attentation_columns[1]].str.extract('(\d\.\d)', expand=True)
		# print(self.data[special_attentation_columns[1]])

		# Extract transimission speed
		self.data['transmission_speeds'] = self.data[special_attentation_columns[2]].str.extract('(\d+)', expand=True)
		...
		trans_type_temp = self.data[special_attentation_columns[2]].str.extract(
			'((?:\s|^)Automatic(?:\s|$))|((?:\s|^)Manual(?:\s|$))', 
			expand=True)
		self.data['transmission_type'] = trans_type_temp[0].fillna(trans_type_temp[1])
		...
		self.data[special_attentation_columns[2]] = self.data[special_attentation_columns[2]].str.replace('Spd', 'Speed')
		self.data[special_attentation_columns[2]] = self.data[special_attentation_columns[2]].str.replace('w/Manual', 'Manual')
		self.data[special_attentation_columns[2]] = self.data[special_attentation_columns[2]].str.replace('w/Automatic', 'Automatic')
		self.data[special_attentation_columns[2]] = self.data[special_attentation_columns[2]].str.replace('w/OD', '')
		self.data[special_attentation_columns[2]] = self.data[special_attentation_columns[2]].str.strip()
	...
	def add_timestamp_columns(self):
		...
		for i in range(len(timestamp_columns)):
			...
			self.data[timestamp_columns[i]] = self.data[timestamp_columns[i]].apply(str)
		...
		for i in range(len(colour_content)):
			...
			if pd.isnull(colour_column.iloc[i]):
				...
				if pd.isnull(gerneic_colour_column.iloc[i]):
					colour_temp = [colour for colour in colour_content.iloc[i] if colour in GENERIC_COLOUR_NAMES]
	...
	def get_colour_columns(self):
		...
		self.data[colour_columns[2]] = self.get_colour(self.data[colour_columns[0]], self.data[colour_columns[2]])
		self.data[colour_columns[3]] = self.get_colour(self.data[colour_columns[1]], self.data[colour_columns[3]])
	...
	def capitalize_columns(self):
		...
		for i in range(len(parse_to_capitalization_columns)):
			self.data[parse_to_capitalization_columns[i]] = self.data[parse_to_capitalization_columns[i]].str.title()
	...
	def all_cap_columns(self):
		...
		for i in range(len(parse_to_all_caps_columns)):
			self.data[parse_to_all_caps_columns[i]] = self.data[parse_to_all_caps_columns[i]].str.upper()
	...
	def all_lower_columns(self):
		...
		for i in range(len(parse_to_all_lower_cases_columns)):
			self.data[parse_to_all_lower_cases_columns[i]] = self.data[parse_to_all_lower_cases_columns[i]].str.lower()
	...
	def num_columns(self):
		...
		for i in range(len(parse_to_small_int_columns)):
			self.data[parse_to_small_int_columns[i]] = self.data[parse_to_small_int_columns[i]].astype(np.int8)

		for i in range(len(parse_to_big_int_columns)):
			self.data[parse_to_big_int_columns[i]] = self.data[parse_to_big_int_columns[i]].astype(np.int32)

		for i in range(len(parse_to_float_columns)):
			# SQLite float type is np.float32
			self.data[parse_to_float_columns[i]] = self.data[parse_to_float_columns[i]].astype(np.float64)
