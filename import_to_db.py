...
class GetDatabase:
    ...
    def update_frame(self, df, name=None, conn=None):
        ...
        try:
            for i in range(len(data[0])):
                update_sql = 'UPDATE %s SET {} = ? WHERE vin = ?'.format(
                    data[i][0]) % (name)
    ...
    def update_database(self):
        ...
        for db_data in pd.read_sql_query(
                sql, connection, chunksize=self.number_lines_read):
            ...
            self.csv_data = self.csv_data[self.csv_data['vin'].isin(
                dup_vin) == False]
        ...
        self.csv_data['v_id'] = range(
            last_v_id_stored + 1, last_v_id_stored + 1 + len(self.csv_data['v_id']))
        # Clean out empty strings
        self.csv_data.replace(r'^\s*$', np.nan, regex=True, inplace=True)

        # Upload the rest of the .csv file
        self.csv_data.to_sql(
            'inventory',
            connection,
            index=False,
            if_exists='append')
